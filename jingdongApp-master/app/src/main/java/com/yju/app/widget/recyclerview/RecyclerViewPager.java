package com.yju.app.widget.recyclerview;
...
public class RecyclerViewPager extends RecyclerView {
    ...
    private float mTriggerOffset = 0.1f;
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyle) {
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RecyclerViewPager, defStyle,
                0);
        mFlingFactor = a.getFloat(R.styleable.RecyclerViewPager_rvp_flingFactor, 0.10f);
            ...
            LinearSmoothScroller linearSmoothScroller =
                    new LinearSmoothScroller(getContext()) {
                        ...
                        public int calculateDxToMakeVisible(View view, int snapPreference) {
                            ...
                            int dt = calculateDtToFit(left, right, start, end, snapPreference);
                            return dt;
                        }

                        public int calculateDtToFit(int viewStart, int viewEnd, int boxStart, int boxEnd, int
                                snapPreference) {
                            int span = boxEnd-boxStart-viewEnd+viewStart;
                            span/=2;
                            boxStart+=span;
                            boxEnd-=span;
    ...
    public void onScrolled(int dx, int dy) {
        ...
        for (int j = 0; j < childCount; j++) {
            ...
            if (v.getLeft() <= padding) {
                ...
                ViewHelper.setScaleY(v,1f - rate * 0.1f);
                ViewHelper.setScaleX(v,1F - rate * 0.1f);
            } else {
                //往右 从 padding 到 recyclerView.getWidth()-padding 的过程中，由大到小
                if (v.getLeft() <= getWidth() - padding) {
                    rate = (getWidth() - padding - v.getLeft()) * 1f / v.getWidth();
                }
                ViewHelper.setScaleY(v,0.9f + rate * 0.1f);
                ViewHelper.setScaleX(v,0.9f + rate * 0.1f);
            }
        }
    }
