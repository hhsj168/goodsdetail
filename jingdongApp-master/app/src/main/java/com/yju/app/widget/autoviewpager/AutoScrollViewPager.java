package com.yju.app.widget.autoviewpager;
...
public class AutoScrollViewPager extends ViewPager {
    ...
    private boolean shouldFitChildHeight = false;
...
//                getParent().requestDisallowInterceptTouchEvent(true);
    ...
    public void setShouldFitChildHeight(boolean fitChildHeight) {
        this.shouldFitChildHeight = fitChildHeight;
    }

    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        if (shouldFitChildHeight && getChildCount() > 0) {
            int targetHeight = getChildAt(0).getMeasuredHeight();
            setMeasuredDimension(widthMeasureSpec, MeasureSpec.makeMeasureSpec(targetHeight, MeasureSpec.EXACTLY));
        }
    }
